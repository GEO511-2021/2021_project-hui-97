...
* Surface ice velocity maps (GeoTIFFs) with 100m x 100m grids and sporadic temporal resolution from NSIDC: https://nsidc.org/data/nsidc-0481/versions/2
* Monthly runoff and air temperature data (netCDF) at 1km x 1km grids are from RACMO2.3p2: https://www.projects.science.uu.nl/iceclimate/models/greenland.php
* Drainage basin outline (shapefiles) from NASA: https://earth.gsfc.nasa.gov/cryo/data/polar-altimetry/antarctic-and-greenland-drainage-systems
...
## Surface ice velocity variation in Jakobshavn Isbrae

```{r, message=F, warning=F, fig.width=6, fig.height=4, fig.cap="Monthly mean surface ice velocity in 2016"}
...
bed = raster("data/bed_Jak_egm08.tif")
leaflet() %>% 
  addTiles(group = "OSM") %>%
...
```{r, message=F, warning=F, fig.width=6, fig.height=6, fig.cap="Time series of the surface ice velocity at the 6 example locations"}
...
## Meltwater runoff variation in the Jakobshavn Isbrae drainge basin

```{r, message=F, warning=F, fig.width=6, fig.height=6, fig.cap="Comparison of time series of surface ice velocity and total runoff in the ablation zone"}
...
ggplot(vel_runoff, aes(x = runoff, y = value)) +
  ...
  theme_bw() +
  labs(x = "Runoff (mm.w.e./month, logscale)", y = "Surface ice velocity (m/year)")
```

```{r, message=F, warning=F, fig.width=6, fig.height=4, fig.cap="Monthly mean meltwater runoff in 2016"}
...
## Air temperature on the Greenland ice sheet

```{r, message=F, warning=F, fig.width=6, fig.height=7, fig.cap="Monthly mean air temperature at 2m above ice surface in 2016"}
...
```{r, message=F, warning=F, fig.width=6, fig.height=4, fig.cap="Correlation map of surface ice velocity and runoff in every pixel"}
